...
public interface Schedule extends Serializable {
	 ...
	 */
	static Schedule parse(String schedule) {
		return ScheduleImpl.parse(schedule);
	}
	 ...
	 */
	static FromScheduleBuilder from(LocalTime start, LocalTime end) {
		return ScheduleImpl.from(start, end);
	}
	 ...
	 * @throws InvalidScheduleException in case of invalid schedule
	 */
	static Schedule every(long length, ChronoUnit unit) {
		return ScheduleImpl.every(length, unit);
	}
	 ...
	 * @throws InvalidScheduleException in case of invalid schedule
	 */
	static Schedule every(long length, ChronoUnit unit, boolean sync) {
		return ScheduleImpl.every(length, unit, sync);
	}

	static AtScheduleBuilder at(LocalTime time) {
		return ScheduleImpl.at(time);
	}

	default ZonedDateTime nextOrSame(ZonedDateTime timestamp) {
		return iterate(timestamp).nextOrSame();
	}

	default ZonedDateTime next(ZonedDateTime timestamp) {
		return iterate(timestamp).next();
	}
	...
	interface AtScheduleBuilder {
		...
		default Schedule everyDay(Month month, Month... months) {
			return everyDay(EnumSet.of(month, months));
		}
	...
	interface NthScheduleBuilder {
		...
		default Schedule of(Month month, Month... months) {
			return of(EnumSet.of(month, months));
		}
	}
...
class $Set {
	static Set<Integer> of(Integer... values) {
