...
internal object ScheduleImpl {
	...
	private const val ORDINAL0 = "\\d\\d?(?:st|nd|rd|th)?|[a-z]+"
	private const val ORDINAL00 = "(?:$ORDINAL0)(?:-(?:$ORDINAL0))?"
	private const val ORDINAL = "(?:$ORDINAL00)(?:,(?:$ORDINAL00))*"
	private const val DAY0 = "[a-z]+"
	private const val DAY00 = "$DAY0(?:-$DAY0)?"
	private const val DAY = "(?:$DAY00)(?:,(?:$DAY00))*"
	private const val MONTH0 = "[a-z]+"
	private const val MONTH00 = "$MONTH0(?:-$MONTH0)?"
	private const val MONTH = "(?:$MONTH00)(?:,(?:$MONTH00))*"
	...
	private val DAY_OF_WEEK_MAP = (
			// mon, ...
			DayOfWeek.values().map { it.name.lowercase().substring(0, 3) to it } +
					// monday, ...
					DayOfWeek.values().map { it.name.lowercase() to it }
			).toMap()
	private val MONTH_MAP = (
			// mar, ...
			Month.values().map { it.name.lowercase().substring(0, 3) to it } +
					// march, ...
					Month.values().map { it.name.lowercase() to it }
			).toMap()
			...
			exp.startsWith(EVERY_PREFIX) && exp.length > EVERY_PREFIX.length &&
				...
				TimeSchedule(
						weekDayIndexesInAMonth = if (day == "" || rawOrdinal == "every") WEEK_ALL else
							rawOrdinal.split(',').flatMapTo(mutableSetOf()) {
	...
	private class IntervalScheduleBuilder(
	...
	private class TimeScheduleBuilder(private val time: LocalTime) : Schedule.AtScheduleBuilder {
	...
	private class NthDayOfWeekScheduleBuilder(
	...
	private class NthDayOfMonthScheduleBuilder(
			private val time: LocalTime,
	...
	private abstract class AbstractSchedule : Schedule {
		...
		override fun iterate(timestamp: ZonedDateTime): Schedule.ScheduleIterator<ZonedDateTime> {
			...
			return object : Schedule.ScheduleIterator<ZonedDateTime> {
				...
				override fun nextOrSame(): ZonedDateTime {
					cursor = nextOrSame(cursor); return cursor
				}

				override fun next(): ZonedDateTime {
					cursor = next(cursor); return cursor
				}
	...
	// "every" N ...
	private class IntervalSchedule(
	...
	fun ZonedDateTime.withTime(value: LocalTime): ZonedDateTime = this.withHour(value.hour).withMinute(value.minute)

	// ("every"|ordinal) ...
	private class TimeSchedule(
		...
		init {
			validate(weekDayIndexesInAMonth.isNotEmpty() && weekDayIndexesInAMonth.all { it in WEEK_RANGE }
			...
			validate(days.isNotEmpty() && days.all { it in DAY_RANGE }
			) { "Invalid day(s) (${days.filter { it !in DAY_RANGE }})" }
			validate(!months.isNotEmpty()
		...
		override val lazyString = lazy(LazyThreadSafetyMode.PUBLICATION) {
			...
			when {
				...
				weekDayIndexesInAMonth == WEEK_ALL && months == MONTH_ALL ->
					...
					if (weekDayIndexesInAMonth == WEEK_ALL) "every" else weekDayIndexesInAMonth.joinToString(",")
				} ${weekDays.joinSorted()} of ${months.joinSorted()} $time"
			}
		}

		private fun <T : Enum<T>> Set<T>.joinSorted() =
				this.sorted().joinToString(",") { it.toString().lowercase() }
